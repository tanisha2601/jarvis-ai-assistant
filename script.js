  ...
  export() {
    ...
    keys.forEach((k) => {
      try {
        ...
        data[k.slice(this.PREFIX.length)] = localStorage.getItem(k);
      }
    });
    return data;
  },
  ...
  _setStatus(state, aiAvailable) {
    ...
    if (state === "online") {
      ...
      if (this._lastOnline === false)
        Toast.success("JARVIS backend connected!");
      ...
      if (this._lastOnline === true)
        Toast.error("Backend offline — start the JARVIS server to continue.");
...
const Chat = {
  ...
  async send(text) {
    ...
    if (!STATE.backendOnline) {
      ...
      this.appendMessage(
        "bot",
        "Backend is offline. Start the JARVIS server and try again.",
        true,
      );
      return;
    }
    ...
    try {
      ...
      const reply =
        data?.text || "I received your message but had no response.";
  ...
  async _autoSaveHistory() {
    ...
    const toSave = STATE.chatMessages
      .slice(-CONFIG.MAX_HISTORY_SAVE)
      .map((m) => ({
...
const Voice = {
  ...
  _syncTTSUI() {
    ...
    if (icon) {
      icon.setAttribute(
        "data-lucide",
        STATE.ttsEnabled ? "volume-2" : "volume-x",
      );
...
const Tasks = {
  ...
  render() {
    ...
    if (this._priorityFilter)
      tasks = tasks.filter((t) => t.priority === this._priorityFilter);
    ...
    if (label)
      label.textContent =
        all.length === 0
          ? "No tasks yet"
          : `${pending} pending · ${all.length - pending} completed`;
    ...
    container.querySelectorAll("[data-delete-task]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        if (ok) {
          this.delete(btn.dataset.deleteTask);
  ...
  render() {
    ...
    grid.querySelectorAll("[data-edit-note]").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        NoteModal.open(btn.dataset.editNote);
      });
    });
    grid.querySelectorAll("[data-delete-note]").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        ...
        if (ok) {
          this.delete(btn.dataset.deleteNote);
...
const NoteModal = {
  ...
  close() {
    DOM.noteModalBackdrop().style.display = "none";
  },
  ...
  render() {
    ...
    container.querySelectorAll("[data-delete-reminder]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        if (ok) {
          this.delete(btn.dataset.deleteReminder);
...
const ReminderModal = {
  ...
  close() {
    DOM.reminderModalBackdrop().style.display = "none";
  },
  ...
  save() {
    ...
    if (!time) {
      Toast.warning("Please choose a time.");
      return;
    }
    ...
    Reminders.create({
      ...
      date,
      time,
      description: DOM.reminderDesc().value,
    });
...
const History = {
  async load() {
    ...
    if (!STATE.backendOnline) {
      container.innerHTML =
        '<div class="empty-state-mini">Backend offline — history unavailable.</div>';
      return;
    }
    ...
    try {
      ...
      container.innerHTML =
        '<div class="empty-state-mini">Could not load history from server.</div>';
    }
  },
  ...
  async clear() {
    if (!STATE.backendOnline) {
      Toast.error("Backend offline — cannot clear server history.");
      return;
    }
    const ok = await Confirm.show(
      "Clear all conversation history from the server? This cannot be undone.",
      "Clear History",
    );
    if (!ok) return;
    try {
    ...
    this._timer = setTimeout(() => {
      ...
      Notes.getAll().forEach((n) => {
        if (
          ...
          n.content.toLowerCase().includes(q2)
        ) {
          hits.push({
            type: "note",
            icon: "file-text",
...
const Dashboard = {
  refresh() {
    ...
    if (el("dashTasksValue"))
      el("dashTasksValue").textContent =
        tasks.length === 0 ? "—" : `${completed}/${tasks.length}`;
    if (el("dashNotesValue"))
      el("dashNotesValue").textContent =
        notes.length === 0 ? "—" : String(notes.length);
    if (el("dashRemindersValue"))
      el("dashRemindersValue").textContent = String(upcoming) || "—";
    ...
    if (recent) {
      ...
      if (top.length === 0) {
        recent.innerHTML =
          '<div class="empty-state-mini">No tasks yet — create one above</div>';
      } else {
...
const Commands = {
  handle(text) {
    ...
    // Navigation
    if (t === "show tasks" || t === "my tasks") {
      Nav.go("tasks");
      Chat._appendInfo("Opening Tasks.");
      return true;
    }
    if (t === "show notes" || t === "my notes") {
      Nav.go("notes");
      Chat._appendInfo("Opening Notes.");
      return true;
    }
    if (t === "show reminders") {
      Nav.go("reminders");
      Chat._appendInfo("Opening Reminders.");
      return true;
    }
    if (t === "show history" || t === "history") {
      Nav.go("history");
      Chat._appendInfo("Opening History.");
      return true;
    }
    if (t === "dashboard") {
      Nav.go("dashboard");
    ...
    if (t === "start voice mode" || t === "voice mode") {
...
const Settings = {
  load() {
    ...
    const vt = DOM.voiceToggleSetting();
    if (vt) vt.checked = STATE.ttsEnabled;
    const vr = DOM.voiceRate();
    if (vr) vr.value = STATE.speechRate;
    const vv = DOM.voiceVolume();
    if (vv) vv.value = STATE.speechVolume;
    const es = DOM.enterToSendToggle();
    if (es) es.checked = STATE.enterToSend;
    const as = DOM.autoScrollToggle();
    if (as) as.checked = STATE.autoScroll;

    const rvl = DOM.voiceRateVal();
    if (rvl) rvl.textContent = STATE.speechRate.toFixed(1) + "×";
    const vvl = DOM.voiceVolumeVal();
    if (vvl) vvl.textContent = Math.round(STATE.speechVolume * 100) + "%";
  ...
  import(file) {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const ok = await Confirm.show(
          "Import will overwrite your current data. Continue?",
          "Import Data",
        );
        if (!ok) return;
        Storage.import(data);
  ...
  async clearAll() {
    const ok = await Confirm.show(
      "This will permanently delete all your tasks, notes, reminders, and preferences. This cannot be undone!",
      "Clear All Data",
    );
    if (!ok) return;
    Storage.clear();
...
function bindEvents() {
  ...
  document.querySelectorAll("[data-action]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (a === "ask-jarvis") {
        Nav.go("chat");
        setTimeout(() => DOM.userInput()?.focus(), 50);
      }
      if (a === "voice-mode") {
        Nav.go("chat");
        Voice.startListening();
  ...
  DOM.taskModalBackdrop()?.addEventListener("click", (e) => {
    if (e.target === DOM.taskModalBackdrop()) TaskModal.close();
  });
  DOM.taskTitle()?.addEventListener("keydown", (e) => {
    if (e.key === "Enter") TaskModal.save();
  });
  ...
  document.querySelectorAll(".filter-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (btn.dataset.priority !== undefined) {
        ...
        document
          .querySelectorAll(".filter-btn[data-priority]")
          .forEach((b) => b.classList.remove("active"));
  ...
  DOM.noteModalBackdrop()?.addEventListener("click", (e) => {
    if (e.target === DOM.noteModalBackdrop()) NoteModal.close();
  });
  DOM.notesSearch()?.addEventListener("input", (e) => {
    Notes._query = e.target.value;
    Notes.render();
  });
  ...
  DOM.closeReminderModal()?.addEventListener("click", () =>
    ReminderModal.close(),
  );
  DOM.cancelReminderModal()?.addEventListener("click", () =>
    ReminderModal.close(),
  );
  DOM.reminderModalBackdrop()?.addEventListener("click", (e) => {
    if (e.target === DOM.reminderModalBackdrop()) ReminderModal.close();
  });
  ...
  DOM.voiceVolume()?.addEventListener("input", (e) => {
    ...
    DOM.voiceVolumeVal().textContent =
      Math.round(STATE.speechVolume * 100) + "%";
  });
...
function init() {
  ...
  console.log(
    "%cJARVIS Ultra Pro",
    "color:#00d4ff;font-family:monospace;font-size:1.2em;font-weight:bold",
  );
